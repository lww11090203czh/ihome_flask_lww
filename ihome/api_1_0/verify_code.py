...
from flask import g, current_app,jsonify,make_response,request
...
def get_sms_code(mobile):
    ...
    #  与用户填写的值做对比
    real_image_code1 = real_image_code
    real_image_code_str = str(real_image_code1,'utf-8')
    # print(real_image_code_str)
    # print('****')

    # image_code1 = image_code
    # image_code_str = str(image_code1,'utf-8')
    # # print(image_code_str)
    ...
    try:
        ...
        if send_flag is not None:
            # 表示在60s内之前有过发送的记录
            return jsonify(errno=RET.REQERR,errmsg="请求过于频繁，请60s后重新试")
    ...
    try:
        ...
        # 保存发送到这个手机号的记录，防止用户在60s内再次出发发送短信的操作
        redis_store.setex("send_sms_code_%s" % mobile,constants.SEND_SMS_CODE_INTERVAL,1)

    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR,errmsg="保存到短信验证码异常")
    #  发送短信
    try:
        ccp = CCP()
...
#
#     #  发送短信
#     # 使用过celery异步发送短信,delay函数调用后立即返回
...
#     # 返回的是异步执行结果对象
#     print(result.id)
#     # 通过get方法可以获取celery异步执行的结果
#     # get方法默认是阻塞的行为，会等到有了执行结果之后才可以返回
#     # get方法也接受参数timeout，超时时间还拿不到结果，则返回
#     ret = result.get()
#     print(ret)
#
#
